...
export default function ModuleCard({ module }: Props) {
  ...
  return (
    ...
    >
      ...
      <p className="line-clamp-2 mt-2 text-sm text-text-secondary">
        {module.summary}
      </p>

      <div className="mt-4 flex flex-wrap gap-1.5">
        {mainstreamCount > 0 && (
          <span className="rounded-full bg-layer-4/10 text-layer-4 text-[11px] px-2 py-0.5">
            {mainstreamCount}个方案
          </span>
        )}
        {module.tags.slice(0, 3).map((tag) => (
          <span
            key={tag}
            className="rounded-full bg-white/5 text-text-muted text-[11px] px-2 py-0.5"
          >
            {tag}
          </span>
        ))}
